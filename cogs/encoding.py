...
type EncOrDec = Literal['encode', 'decode']
...
class Encoding(commands.Cog):
    ...
    @commands.command()
    async def b64(self, ctx: Context, encode_or_decode: EncOrDec, string: str):
    ...
    @commands.command()
    async def b32(self, ctx: Context, encode_or_decode: EncOrDec, string: str):
        byted_str = str.encode(string)
    ...
    @ commands.command()
    async def binary(self, ctx: Context, encode_or_decode: EncOrDec, string: str):
        if encode_or_decode == 'decode':
    ...
    @ commands.command()
    async def hex(self, ctx: Context, encode_or_decode: EncOrDec, string: str):
        if encode_or_decode == 'decode':
            string = string.replace(" ", "")
    ...
    @ commands.command()
    async def url(self, ctx: Context, encode_or_decode: EncOrDec, message: str):
        if encode_or_decode == 'decode':
